...
status_reply_likes = db.Table('status_reply_likes',
                              db.Column('status_reply_id', db.Integer, db.ForeignKey('status_replies.id'),
                                        primary_key=True),
                              db.Column('user_id', db.Integer, db.ForeignKey('users.id'), primary_key=True))
...
class StatusReply(db.Model):
    ...
    status_id = db.Column(db.Integer, db.ForeignKey('statuses.id'), nullable=False)  # 懒加载, 访问到属性的时候, 就会加载该属性的全部数据
...
class Status(db.Model):
    ...
    def process_json(json_status):
        ...
        json_status['liked_by_me'] = Cache.is_status_liked_by(id, g.user.id) if hasattr(g, 'user') else False
